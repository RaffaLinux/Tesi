...
def find_max_clustering(clusters, device, algorithm, dataset, initial_clusters):

   new_clusters = clusters
   cluster_degenere = False
   i = initial_clusters+1
   bad_start = check_cluster_degeneri(clusters)
   if bad_start:
      print("\nI cluster di partenza già hanno dei cluster degeneri")
      print(clusters)
      return
   while i < CLUSTERS_MAX and cluster_degenere == False:
