...
def load_features_clusters(features_clusters, device: Device):
   ...
   dataframe_to_feature_clusters(df_kshape, features_clusters['Kshape'])
   dataframe_to_feature_clusters(df_kernel, features_clusters['KernelKmeans'])
   dataframe_to_feature_clusters(df_kmeans, features_clusters['Kmeans'])
...
for n_autoencoder in (5,10,15,20):
   ...
   for train_index, test_index in tss.split(device_benign, device_benign[:,116]):
      with tf.device('/cpu:0'):
         ...
         testing = device_benign[test_index, : 117]
         test_features = device_all[test_index, : 115]
         test_labels = device_all[test_index, 115:117]
         ...
         for train_index_skf, test_index_skf in skf.split(device_malign, device_malign[:,115]):
            ennio_mix = np.concatenate([testing, device_malign[test_index_skf, : 116]], axis = 0)
