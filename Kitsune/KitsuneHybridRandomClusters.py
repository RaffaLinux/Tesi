...
def get_random_clusters(n_device, n_elem, random_state=0):
   if n_elem < 8:
      ...
      c=[]
      c.append([])
      c[-1].append(a.pop(list(range(len(a)))[random_state]))
...
for n_clusters in [4]:
   for n_restart in range(0,10):
      ...
      for devices_list in get_random_clusters(9,n_clusters,n_restart):
         ...
         devices_list_str = devices_list_str[:-3]
         ...
         for train_index, test_index in skf.split(all_devices_mix, all_devices_mix[:,116]):
            with tf.device('/cpu:0'):
               ...
               StampaValori(devices_list_str,n_clusters, tss_iteration, test_index, test_labels, RMSE, n_restart)
